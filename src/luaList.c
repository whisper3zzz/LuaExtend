...
const char *const LIST_KEY = "_LIST";
...
static int list_new(lua_State *L) {
  ...
  ls->size = 0;
  ls->ref = 0;
  ls->ary = (refdata_t *)_malloc(ls->cap * sizeof(refdata_t));
...
static void check_and_grow_size(list_t *ls, int n) {
  ...
  while (ls->size + n > newcap)
    newcap <<= 1;
  if (newcap != ls->cap) {
    ls->ary = (refdata_t *)_realloc(ls->ary, newcap * sizeof(refdata_t));
    ls->cap = newcap;
  }
}
...
static int list_insert(lua_State *L) {
  ...
  if (pos != ls->size)
    memmove(ls->ary + pos + 1, ls->ary + pos,
            (ls->size - pos) * sizeof(refdata_t));
...
static int list_remove(lua_State *L) {
  ...
  if (pos != ls->size - 1)
    memmove(ls->ary + pos, ls->ary + pos + 1,
            (ls->size - pos - 1) * sizeof(refdata_t));
  ls->size--;
...
static int list_clear(lua_State *L) {
  ...
  if (shink) {
    ls->cap = 4;
    ls->ary = (refdata_t *)_realloc(ls->ary, ls->cap * sizeof(refdata_t));
  }
...
static void addfield(lua_State *L, luaL_Buffer *b, lua_Integer i, int uvidx,
                     int ref) {
...
#define sort_comp(L, mode, a, b)                                               \
  mode == M_KEY ? (int)(a.key < b.key) : _sort_comp(L, mode, a, b)
...
static int partition(lua_State *L, int mode, refdata_t *ary, int lo, int up,
                     int p) {
...
static void quick_sort(lua_State *L, int mode, refdata_t *a, int lo, int up,
                       unsigned int rnd) {
  int p;
  int n;
  while (lo < up) {
...
LUAOPEN int luaopen_LuaExtendLib_list(lua_State *L) {
  luaL_checkversion(L);
