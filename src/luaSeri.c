 ...
 * 		| type  cookie | objdata |
 * 		type 和 cookie
 * 合并为一个字节；type表示类型，占低3位；cookie为类型附带的一些信息，占高5位。
 * 		objdata 是对象相关的数据，有些类型通过 type+cookie
 * 就能表达，所以objdata不一定存在。
 *
 * 		type == TYPE_NIL 表示 nil，不需要cookie和objdata。
 *
 * 		type == TYPE_BOOL 表示 boolean， cookie为0表示false,
 * 为1表示true；不需要objdata。
 *
 * 		type == TYPE_INT 表示 integer：
 * 			cookie == TYPE_INT_0 表示0；不需要objdata。
 * 			cookie == TYPE_INT_8
 * 表示int8；objdata为1个字节的小端int8数据。 cookie == TYPE_INT_16
 * 表示int16；objdata为2个字节的小端int16数据。 cookie == TYPE_INT_32
 * 表示int32；objdata为4个字节的小端int32数据。 cookie == TYPE_INT_64
 * 表示int64；objdata为8个字节的小端int64数据。
 *
 * 		type == TYPE_FLOAT
 * 表示float，不需要cookie；objdata为8个字节的小端double数据
 *
 * 		type == TYPE_SSTR
 * 表示短字符串；cookie为字符串长度；objdata为字符串数据
 *
 ...
 * 			cookie == 31
 * 表示数组长度在objdata中；objdata存：数组长度 + 数组元素 + 哈希表
 * 				数组长度的存储格式是 TYPE_INT
 *
 * 			数组元素为按顺序存储的对象。
 ...
 *
 * 		type = TYPE_USERDATA
 * 表示lightuserdata，由于是一个指针，所以不可以跨进程传输；不需要cookie；objdata是一个指针
 *
...
#define TYPE_BOOL 1     // 布尔类型
#define TYPE_INT 2      // 整数类型
#define TYPE_FLOAT 3    // 浮点数类型
#define TYPE_SSTR 4     // 短字符串
#define TYPE_LSTR 5     // 长字符串
#define TYPE_TABLE 6    // 表类型
#define TYPE_USERDATA 7 // lightuserdata类型

// 整数类型的cookie
#define TYPE_INT_0 0  // 整数0
#define TYPE_INT_8 1  // 8位整数
#define TYPE_INT_16 2 // 16位整数
#define TYPE_INT_32 3 // 32位整数
#define TYPE_INT_64 4 // 64位整数

#define MAX_COOKIE 31                             // cookie最大值
#define COMBINE_TYPE(t, v) (char)((t) | (v) << 3) // 合并type和cookie
#define TYPE_TYPE(t) ((t) & 7)                    // 获取type
#define TYPE_COOKIE(t) (((t) >> 3) & 31)          // 获取cookie

#define MAX_DEPTH 32 // 最大递归深度
...
static const union {
  int dummy;
  char little; /* 如果是小端序则为true */
} nativeendian = {1};

//---------------------------------------------------------------------------------------------
// pack（序列化相关）
#define INIT_BUFF_SIZE 512 // 初始缓冲区大小
...
static char *wb_prepbuffsize(WriteBuffer *b, size_t sz) {
  if (b->size + sz > b->cap) {
    ...
    if (newcap < b->size + sz)
      newcap = b->size + sz;
    if (b->buff == b->b) {
      b->buff = (char *)_malloc(newcap);
      memcpy(b->buff, b->b, b->size * sizeof(char));
    } else {
      b->buff = (char *)_realloc(b->buff, newcap);
    }
    b->cap = newcap;
  }
...
// 序列化哈希部分
static void pack_hash(lua_State *L, WriteBuffer *b, int index, int depth,
                      int array_size) {
  lua_pushnil(L);
  ...
  wb_addchar(b, TYPE_NIL); // 哈希部分结束标记
}
...
// 预读取指定长度的数据
static inline const char *rb_prepbuffsize(lua_State *L, ReadBuffer *b,
                                          size_t sz) {
  if (b->pos + sz <= b->len) {
...
static lua_Integer unpack_int(lua_State *L, ReadBuffer *b, int cookie) {
  ...
  case TYPE_INT_0:
    return 0;
  case TYPE_INT_8:
    return (lua_Integer)unpack_intx(L, b, int8_t);
  case TYPE_INT_16:
    return (lua_Integer)unpack_intx(L, b, int16_t);
  case TYPE_INT_32:
    return (lua_Integer)unpack_intx(L, b, int32_t);
  case TYPE_INT_64:
    return (lua_Integer)unpack_intx(L, b, int64_t);
  default:
    invalid_stream(L, b);
  }
  return 0;
...
static void unpack_table(lua_State *L, ReadBuffer *b, int cookie) {
  ...
  while (1) {
    ...
    // 如果不是nil，说明还有键值对需要处理
    if (!lua_isnil(L, -1)) {
      unpack_value(L, b); // 反序列化value
      lua_rawset(L, -3);  // 设置table[key]=value
    } else {
      lua_pop(L, 1);
      break;
    }
...
// Lua模块入口
LUAOPEN int luaopen_LuaExtendLib_seri(lua_State *L) {
  luaL_newlib(L, lib);
  return 1;
}
